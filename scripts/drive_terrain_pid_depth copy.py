...
- Debug draw arrow showing camera position and direction in viewport
...
DEFAULT_USD = (
    ...
    / "Robots" / "xlerobot" / "xlerobot_wheel_v13.usd"
)
...
# Camera arguments
parser.add_argument("--cam_x", type=float, default=0.3, help="Camera X offset from base_link (m).")
parser.add_argument("--cam_z", type=float, default=0.3, help="Camera Z offset from base_link (m).")
parser.add_argument("--cam_pitch", type=float, default=20.0, help="Camera downward pitch (degrees).")
...
try:
    ...
    print("[WARN] omni.isaac.debug_draw not available — camera arrow disabled.")
...
def build_camera_cfg() -> RayCasterCameraCfg:
    ...
    half = pitch_rad / 2.0
    qw, qx, qy, qz = math.cos(half), 0.0, math.sin(half), 0.0
    return RayCasterCameraCfg(
        ...
        offset=RayCasterCameraCfg.OffsetCfg(
            ...
            rot=(qw, qx, qy, qz), convention="world",
        ),
...
def depth_to_colormap(depth, max_dist):
    depth_norm = np.clip(depth / max_dist, 0.0, 1.0)
...
def main():
    ...
    debug_draw = _debug_draw.acquire_debug_draw_interface() if HAS_DEBUG_DRAW else None
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            # ── Camera arrow in viewport ──
            if debug_draw is not None:
                cam_pos = depth_camera.data.pos_w[0].cpu()
                cam_quat = depth_camera.data.quat_w_world[0].cpu()
                w, x, y, z = cam_quat[0].item(), cam_quat[1].item(), cam_quat[2].item(), cam_quat[3].item()
                # Forward = -Z in world camera convention
                fx = -2*(x*z + w*y);  fy = -2*(y*z - w*x);  fz = -(1 - 2*(x*x + y*y))
                origin = (cam_pos[0].item(), cam_pos[1].item(), cam_pos[2].item())
                tip = (origin[0]+fx*1.5, origin[1]+fy*1.5, origin[2]+fz*1.5)
                debug_draw.clear_lines()
                debug_draw.draw_lines([origin], [tip], [(1.0, 0.0, 0.0, 1.0)], [4.0])
                # Arrowhead
                ux = 2*(x*y + w*z);  uy = 1-2*(x*x + z*z);  uz = 2*(y*z - w*x)
                for s in [1, -1]:
                    wing = (tip[0]-fx*0.25+s*ux*0.12, tip[1]-fy*0.25+s*uy*0.12, tip[2]-fz*0.25+s*uz*0.12)
                    debug_draw.draw_lines([tip], [wing], [(1.0, 0.3, 0.0, 1.0)], [3.0])
            ...
            if frame_count % 4 == 0:
                ...
                if depth_data is not None and depth_data.numel() > 0:
                    ...
                    valid = depth_img[depth_img < args_cli.cam_max_dist]
                    if valid.size > 0:
